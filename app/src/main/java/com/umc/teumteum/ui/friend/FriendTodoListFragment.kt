...
import com.umc.teumteum.databinding.FragmentFriendTodoListBinding
import com.umc.teumteum.ui.friend.adapter.PublicTodoAdapter
...
class FriendTodoListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 공개 투두 '점' 날짜 목록 옵저버
        viewModel.publicTodoDotDates.observe(viewLifecycleOwner) { dates: List<LocalDate> ->
            eventDates.clear()
    ...
    private fun onDateSelected(date: LocalDate) {
        selectedDate = date
        if (friendUserId != -1) {
            viewModel.fetchFriendPublicTodosByDate(friendUserId, formatDateForApi(date))
        }
    }
